...
#CVEs with a fix commit in not filtered out (or not in range or diff modify more than 20 files)
#removed "CVE-2015-2156", "CVE-2014-0225", "CVE-2013-6430", "CVE-2015-4165", "CVE-2014-0229", "CVE-2015-1776", "CVE-2016-5001", "CVE-2017-3166", "CVE-2018-8012"
...
for cve in cve_list:
  ...
  for item in spacy_scaled_list:
    similarity = item[1]
    commit_sha = item[0]
    commit_sha_dict[commit_sha]['spacy'] = similarity

  for item in fasttext_scaled_list:
    similarity = item[1]
    commit_sha = item[0]
    commit_sha_dict[commit_sha]['fasttext'] = similarity

  os.chdir(current_working_directory)

  for sha_key in commit_sha_dict.keys():
    spacy_sim = commit_sha_dict[sha_key].get('spacy')
    fasttext_sim = commit_sha_dict[sha_key].get('fasttext')
    results_all_commits_list.append((sha_key, spacy_sim, fasttext_sim, cve))

columns = ['commit_id', 'similarity_spacy', 'similarity_fasttext', 'vulnerability_id']
df_all_commits = pd.DataFrame(results_all_commits_list, columns =columns)
df_all_commits.to_csv('final_results_all_commits_prospector.csv')
